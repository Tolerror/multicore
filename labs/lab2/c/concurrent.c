...
void *work(void *arg){
    ...
    while(true){
        ...
        while(graph->thread_queues[id]->head->next == NULL){
            ...
            if(no_active_threads(graph) && terminate_req(graph)){
        ...
        while(edges != NULL && candidate_edge == NULL){   //race condition when finding a suitable edge and determining u and v
            e = edges->edge;
            edges = edges->next;
